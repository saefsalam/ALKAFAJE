...
class _CartScreenState extends State<CartScreen> {
  ...
  Future<void> _updateQuantity(
      int itemId, int newQuantity, bool isLoggedIn) async {
    bool success;
    if (isLoggedIn) {
          ...
          return Scaffold(
            ...
            body: Stack(
              children: [
                Positioned.fill(
                    child:
                        Image.asset('assets/img/main.png', fit: BoxFit.cover)),
                const Center(child: CenteredLoading()),
              ],
            ),
          );
        }
  ...
  Widget _buildBottomBar(double totalPrice,
      List<Map<String, dynamic>> cartItems, bool isLoggedIn) {
    return Container(
      decoration: BoxDecoration(
        color: Colors.white,
                ...
                children: [
                  Text('الإجمالي:',
                      style: GoogleFonts.cairo(
                          ...
                          fontWeight: FontWeight.bold,
                          color: Colors.black87)),
                  Text('${totalPrice.toStringAsFixed(0)} IQD',
                      style: GoogleFonts.cairo(
                          fontSize: 22,
                          fontWeight: FontWeight.bold,
                          color: AppColors.primaryColor)),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                      ...
                      final result = await Navigator.push<bool>(
                          ...
                          MaterialPageRoute(
                              builder: (context) => CheckoutScreen(
                                  cartItems: cartItems, subtotal: totalPrice)));
                      if (result == true && mounted) {
                        _refresh();
                      }
                    }
                  ...
                  style: ElevatedButton.styleFrom(
                      ...
                      elevation: 8),
                  child: Text(isLoggedIn ? 'إتمام الشراء' : 'سجل دخولك للشراء',
                      style: GoogleFonts.cairo(
                          fontSize: 17,
                          fontWeight: FontWeight.bold,
                          color: Colors.white)),
